"""
estudo_taxa_real_tesouro_ipca_vs_fiis_v3_com_ipca_selic_graficos.py

Extensão do estudo anterior:
- Mantém o estudo de taxas reais do Tesouro IPCA+ e comparação com FIIs/carteiras.
- Adiciona IPCA e Selic nos gráficos:
  1) gráfico de taxas do Tesouro: inclui Selic meta (% a.a.) e IPCA mensal (% a.m.)
  2) gráficos de blocos/carteiras: incluem IPCA acumulado e Selic acumulada (base 100)

Instalação:
    pip install pandas matplotlib openpyxl requests yfinance

Execução:
    python estudo_taxa_real_tesouro_ipca_vs_fiis_v3_com_ipca_selic_graficos.py

Saída:
    saida_estudo_taxa_real_tesouro_vs_fiis_v3/
"""
...
SGS_IPCA = 433     # IPCA mensal (% a.m.)
SGS_SELIC_META = 432  # Selic meta (% a.a.) diária
SGS_SELIC_MENSAL = 4390  # Selic acumulada no mês (% a.m.)
...
def fetch_sgs_series(code: int, start: pd.Timestamp, end: pd.Timestamp) -> pd.DataFrame:
    """
    Busca série SGS do Banco Central.

    Algumas séries diárias do BCB, como Selic meta, podem retornar erro 406
    quando o intervalo passa de 10 anos. Para evitar isso, a função baixa em
    pedaços menores e depois concatena.
    """
    frames = []
    ...
    while current <= final:
        chunk_end = min(current + pd.DateOffset(years=5), final)

        url = (
            f"https://api.bcb.gov.br/dados/serie/bcdata.sgs.{code}/dados"
            f"?formato=json&dataInicial={current.strftime('%d/%m/%Y')}&dataFinal={chunk_end.strftime('%d/%m/%Y')}"
        )
        ...
        if raw:
            ...
            frames.append(df[["data", "valor"]])

        current = chunk_end + pd.DateOffset(days=1)

    if not frames:
        raise RuntimeError(f"BCB não retornou dados para a série {code}.")

    out = pd.concat(frames, ignore_index=True)
    out = out.drop_duplicates(subset=["data"]).sort_values("data")
    return out[["data", "valor"]]


def fetch_ipca_monthly(start: pd.Timestamp, end: pd.Timestamp) -> pd.DataFrame:
    df = fetch_sgs_series(SGS_IPCA, start, end)
    df["mes"] = df["data"].dt.to_period("M").dt.to_timestamp()
    df["ipca_mensal_pct"] = df["valor"]
    return df[["mes", "ipca_mensal_pct"]].sort_values("mes")


def fetch_selic_meta_monthly(start: pd.Timestamp, end: pd.Timestamp) -> pd.DataFrame:
    df = fetch_sgs_series(SGS_SELIC_META, start, end)
    df["mes"] = df["data"].dt.to_period("M").dt.to_timestamp()
    # usa o último valor do mês
    out = df.groupby("mes", as_index=False)["valor"].last()
    out.columns = ["mes", "selic_meta_aa_pct"]
    return out


def fetch_selic_monthly(start: pd.Timestamp, end: pd.Timestamp) -> pd.DataFrame:
    df = fetch_sgs_series(SGS_SELIC_MENSAL, start, end)
    df["mes"] = df["data"].dt.to_period("M").dt.to_timestamp()
    out = df.groupby("mes", as_index=False)["valor"].last()
    out.columns = ["mes", "selic_mensal_pct"]
    return out
...
def build_accumulated_series_from_monthly(monthly_df: pd.DataFrame, month_col: str, ret_col: str, start: pd.Timestamp, end: pd.Timestamp, name: str) -> pd.Series:
    use = monthly_df[(monthly_df[month_col] >= start.to_period("M").to_timestamp()) & (monthly_df[month_col] <= end.to_period("M").to_timestamp())].copy()
    use["indice"] = (1 + use[ret_col] / 100).cumprod() * 100
    idx = pd.date_range(start=start, end=end, freq="B")
    monthly = pd.Series(use["indice"].values, index=use[month_col])
...
def plot_taxa_real(monthly: pd.DataFrame, output_path: Path):
    ...
    ax1.plot(monthly["mes"], monthly["taxa_titulo_mais_longo"], linewidth=2, label="Tesouro IPCA+ mais longo (taxa real % a.a.)")
    ax1.plot(monthly["mes"], monthly["taxa_media_ipca_mais"], linewidth=1.6, label="Média taxas reais IPCA+ (% a.a.)")
    if "selic_meta_aa_pct" in monthly.columns:
        ax1.plot(monthly["mes"], monthly["selic_meta_aa_pct"], linewidth=1.5, label="Selic meta (% a.a.)")
    ax1.axhline(TAXA_REAL_ALVO, linestyle="--", linewidth=1.5, label="Linha IPCA+4,5%")
    ax1.set_xlabel("Mês")
    ax1.set_ylabel("Taxas ao ano (%)")
    ax1.grid(True, alpha=0.25)

    ax2 = ax1.twinx()
    if "ipca_mensal_pct" in monthly.columns:
        ax2.plot(monthly["mes"], monthly["ipca_mensal_pct"], linewidth=1.2, linestyle=":", label="IPCA mensal (% a.m.)")
    ax2.set_ylabel("IPCA mensal (%)")

    handles1, labels1 = ax1.get_legend_handles_labels()
    handles2, labels2 = ax2.get_legend_handles_labels()

    ax1.set_title("Taxas reais do Tesouro IPCA+ com Selic e IPCA")
...
def plot_portfolios(df: pd.DataFrame, title: str, output_path: Path):
    ...
    for col in df.columns:
        plt.plot(df.index, df[col], linewidth=2 if "Carteira" in col or "Tesouro" in col else 1.6, label=col)
...
def main():
    ...
    output_dir = Path("saida_estudo_taxa_real_tesouro_vs_fiis_v3")
    ...
    print("Baixando IPCA e Selic no Banco Central...")
    ipca = fetch_ipca_monthly(start_10y - pd.DateOffset(months=2), hoje)
    selic_meta = fetch_selic_meta_monthly(start_10y - pd.DateOffset(months=2), hoje)
    selic_mensal = fetch_selic_monthly(start_10y - pd.DateOffset(months=2), hoje)

    monthly_joined = tesouro_monthly_10y.merge(ipca, on="mes", how="left").merge(selic_meta, on="mes", how="left")
    tesouro_windows = summarize_tesouro_windows(tesouro_monthly_10y, hoje)

    plot_taxa_real(monthly_joined, output_dir / "taxas_reais_tesouro_ipca_mensal_com_ipca_selic.png")
    ...
    for janela, start in janelas.items():
        ...
        ipca_acc = build_accumulated_series_from_monthly(ipca, "mes", "ipca_mensal_pct", start, hoje, "IPCA acumulado")
        selic_acc = build_accumulated_series_from_monthly(selic_mensal, "mes", "selic_mensal_pct", start, hoje, "Selic acumulada")
        ...
        for item, serie in [
            ("IPCA acumulado", ipca_acc),
            ("Selic acumulada", selic_acc),
            ("Ações", acoes),
            ("BDRs", bdrs),
        ...
        df_plot = pd.concat(
            [ipca_acc, selic_acc, acoes, bdrs, fiis, tesouro_45, carteira_25, carteira_301525, carteira_3040, carteira_3530],
            axis=1,
            sort=False,
        ...
        if not df_plot.empty:
            ...
            plot_portfolios(
                df_plot,
                f"IPCA, Selic, blocos e carteiras - {janela}",
                output_dir / f"blocos_e_carteiras_com_ipca_selic_{janela.replace(' ', '_')}.png"
            )

    summary = pd.DataFrame(all_summary)
    details = pd.concat(all_details, ignore_index=True)

    excel_path = output_dir / "estudo_taxa_real_tesouro_ipca_vs_fiis_com_ipca_selic.xlsx"
    with pd.ExcelWriter(excel_path, engine="openpyxl") as writer:
        ...
        selic_mensal.round(6).to_excel(writer, sheet_name="Selic mensal usada", index=False)
