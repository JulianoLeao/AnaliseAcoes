"""
simulacao_tesouro_ipca_vs_carteira_35_35_30_sem_fiis.py
...
Execução:
    python simulacao_tesouro_ipca_vs_carteira_35_35_30_sem_fiis.py

Saída:
    saida_tesouro_ipca_vs_carteira_35_35_30_sem_fiis/
"""
...
def plot_bar(summary_real: pd.DataFrame, output_path: Path) -> None:
    itens = ["Ações", "BDRs", "Tesouro IPCA+4,5%", "Carteira 35/35/30 sem FIIs"]
    ...
    ax.set_title("Retorno real acumulado acima do IPCA - Carteira 35/35/30 sem FIIs")
...
def main() -> None:
    ...
    output_dir = Path("saida_tesouro_ipca_vs_carteira_35_35_30_sem_fiis")
    ...
    for janela, start in janelas.items():
        ...
        for label, serie in [
            ...
            ("Ações", acoes),
            ("BDRs", bdrs),
            ("Carteira 35/35/30 sem FIIs", carteira),
        ]:
        ...
        if not df_plot.empty:
            ...
            plot_lines(
                df_plot,
                f"IPCA, Tesouro IPCA+4,5 e carteira 35/35/30 sem FIIs - {janela}",
    ...
    excel_path = output_dir / "tesouro_ipca_vs_carteira_35_35_30_sem_fiis.xlsx"
