"""
simulacao_tesouro_ipca_vs_carteira_25_25_25_25_sem_xpml.py
...
4) Carteira atual completa em blocos iguais:
   - 25% Ações
   - 25% BDRs
   - 25% FIIs, removendo XPML11
...
- A carteira usa preços ajustados do Yahoo Finance para ações, FIIs e BDRs.
- XPML11 foi removido dos FIIs por distorção de dados.

Instalação:
    pip install yfinance pandas matplotlib openpyxl requests

Execução:
    python simulacao_tesouro_ipca_vs_carteira_25_25_25_25_sem_xpml.py

Saída:
    saida_tesouro_ipca_vs_carteira_sem_xpml/
"""
...
# ============================================================
# Carteira conforme imagens, removendo XPML11 dos FIIs
# ============================================================
...
def summarize_inflation_tesouro(janela: str, years: float, ipca: pd.Series, tesouro: pd.Series) -> dict:
    ...
    # Ganho real acumulado = quanto o Tesouro ficou acima do IPCA.
...
def plot_bar(summary_real: pd.DataFrame, output_path: Path) -> None:
    itens = ["Ações", "BDRs", "FIIs sem XPML11", "Tesouro IPCA+4,5%", "Carteira 25/25/25/25 sem XPML11"]
    ...
    ax.set_title("Retorno real acumulado acima do IPCA")
...
def main() -> None:
    ...
    output_dir = Path("saida_tesouro_ipca_vs_carteira_sem_xpml")
    ...
    for janela, start in janelas.items():
        ...
        carteira = build_weighted_portfolio({
            ...
            "Tesouro IPCA+4,5%": (tesouro_series, 0.25),
        }, "Carteira 25/25/25/25 sem XPML11")
        ...
        for label, serie in [
            ...
            ("Carteira 25/25/25/25 sem XPML11", carteira),
        ]:
            all_summary.append(summarize_series(serie, janela, label, years))

        detalhes = pd.concat([det_acoes, det_bdrs, det_fiis], ignore_index=True)
        all_details.append(detalhes)

        ipca_monthly["Janela"] = janela
        all_ipca_monthly.append(ipca_monthly)

        df_plot = pd.concat([ipca_series, tesouro_series, acoes, bdrs, fiis, carteira], axis=1, sort=False).sort_index().ffill().dropna()
        if not df_plot.empty:
            df_plot = df_plot / df_plot.iloc[0] * 100
            plot_lines(
                df_plot,
                f"IPCA, Tesouro IPCA+4,5 e carteira 25/25/25/25 sem XPML11 - {janela}",
    ...
    excel_path = output_dir / "tesouro_ipca_vs_carteira_25_25_25_25_sem_xpml.xlsx"
